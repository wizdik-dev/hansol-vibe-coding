...
import { getUser, logout, getUnreadCount, getNotifications, markAllRead, markRead, isAdmin } from '../store'
...
export default function Navbar() {
  ...
  useEffect(() => {
    const t = setInterval(refreshNotifs, 5000)
  ...
  function handleMarkAll() { markAllRead(); refreshNotifs() }
    ...
    <nav className={`bg-surface-white/80 glass-nav border-b border-outline-variant fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? 'h-16 shadow-md' : 'h-20'}`}>
      <div className="flex justify-between items-center px-4 md:px-12 h-full w-full max-w-[1280px] mx-auto">
        ...
        <div className="flex items-center gap-10">
          <Link to="/" className="font-headline text-2xl font-bold text-deep-navy tracking-tight">Vibe Coding</Link>
          ...
          {user ? (
            <>
              ...
              <div className="relative" ref={notifRef}>
                <button onClick={() => { setNotifOpen(v => !v); if (!notifOpen) refreshNotifs() }} className="relative p-2 hover:bg-surface-container-low rounded-lg transition-colors">
                ...
                {notifOpen && (
                  <div className="absolute right-0 top-full mt-2 w-80 bg-surface-white border border-outline-variant rounded-xl shadow-2xl overflow-hidden z-50">
                    <div className="flex items-center justify-between px-4 py-3 border-b border-outline-variant bg-surface-container-low">
                      <span className="font-headline text-sm font-bold text-deep-navy">알림 {unread > 0 && <span className="text-error">({unread})</span>}</span>
                      {unread > 0 && <button onClick={handleMarkAll} className="font-label text-xs text-primary hover:underline">모두 읽음</button>}
                    </div>
      ...
      {menuOpen && (
        <div className="md:hidden bg-surface-white border-t border-outline-variant shadow-lg">
          <div className="flex flex-col px-6 py-4 gap-3">
            ...
            {user?.role === 'admin' && <Link to="/admin" onClick={() => setMenuOpen(false)} className="font-label text-sm text-error font-bold py-2">관리자 대시보드</Link>}
